...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        const table = $('#itemTable').DataTable({
            ...
            ajax: async function (data, callback, settings) {
                ...
                var orderableColumns = (checkDeletePermission) ? ['', '', 'name', 'finalPrice', 'category','brand', '', ''] : ['', 'name', 'finalPrice', 'category','brand', '', ''] // Ensure this matches the actual column names
                ...
                await ref.get().then(async function (querySnapshot) {
                    ...
                    await Promise.all(querySnapshot.docs.map(async (doc) => {
                        ...
                        const category = await productCategory(childData.categoryID);
                        var brand = await productBrand(childData.brandID);
                        childData.category = category ? category : "";
                        childData.brand = brand ? brand : "";

                        var dis_price = parseFloat(parseFloat(childData.price) * parseFloat(childData.discount)) / 100;
                        var finalPrice = parseFloat(childData.price) - dis_price;
                        childData.finalPrice = parseInt(finalPrice);
                        if (searchValue) {
                            if (
                                (childData.name && childData.name.toLowerCase().toString().includes(searchValue)) ||
                                (category && category.toLowerCase().toString().includes(searchValue)) ||
                                (brand && brand.toLowerCase().toString().includes(searchValue)) ||
    ...
    async function buildHTML(val) {
        ...
        if (val.hasOwnProperty('discount') && val.discount != '' && val.discount != '0') {
            if (currencyAtRight) {
                html.push('<span class="text-green" data-html="true" data-order="' + dis_price + '">' + parseFloat(dis_price).toFixed(decimal_degits) + '' + currentCurrency + '  <s>' + parseFloat(val.price).toFixed(decimal_degits) + '' + currentCurrency + '</s></span>');
            } else {
                html.push('<span class="text-green" data-html="true" data-order="' + dis_price + '">' + '' + currentCurrency + parseFloat(dis_price).toFixed(decimal_degits) + '  <s>' + currentCurrency + '' + parseFloat(val.price).toFixed(decimal_degits) + '</s></span>');
            }
        } else {
            if (currencyAtRight) {
                html.push('<span class="text-green" data-html="true" data-order="' + val.price + '">' + parseFloat(val.price).toFixed(decimal_degits) + '' + currentCurrency + '</span>');
            } else {
                html.push('<span class="text-green" data-html="true" data-order="' + val.price + '">' + currentCurrency + '' + parseFloat(val.price).toFixed(decimal_degits) + '</span>');
            }
        }
