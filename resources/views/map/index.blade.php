                        ...
                        <div class="col-lg-4">
                            ...
                            <div class="table-responsive ride-list">
                                ...
                                <div class="form-group" id="search-box">
                                    <input type="text" name="search" id="search" class="form-control" style="width:90%"
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var orders = [];
            var orders_drivers = [];
            database.collection('orders').where('status', '==', 'InTransit').get().then(async function (snapshots) {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (driver != '' && driver != undefined) {
                    if (val.flag == "in_transit") {
                        ...
                        let user = await getUserDetail(val.user.id);

                        if (user != undefined && user != '') {

                            if (driver.hasOwnProperty('location') && driver.location.hasOwnProperty('latitude') && driver.location.hasOwnProperty('longitude')) {
                                ...
                                html += '<h3 class="drier-name">{{trans("lang.driver_name")}} : ' + driver.name + '</h3>';
                                if (user.name) {
                                    html += '<h4 class="user-name">{{trans("lang.user_name")}} : ' + user.name + '</h4>';
                                }
                                if (val.user.businessAddress && val.vendor.location) {
                                    html += '<div class="location-ride">';
                                    html += '<div class="from-ride"><span>' + val.vendor.location + '</span></div>';

                                    html += '<div class="to-ride"><span>' + val.user.businessAddress + '</span></div>';
                                    html += '</div>';
                                }
                ...
                if (driver != undefined && driver != '') {
                    if (driver.hasOwnProperty('location') && driver.location.hasOwnProperty('latitude') && driver.location.hasOwnProperty('longitude')) {
                        ...
                        var content = `
                    <div class="p-2">
                        <h6>{{trans('lang.driver_name')}} : ${driver.name ?? '-'} </h6>
                        <h6>{{trans('lang.phone')}} : ${(driver.countryCode) ? driver.countryCode + "" + driver.phoneNumber : driver.phoneNumber ?? '-'} </h6>
